package pl.seb.czech.ilegal.back.client.judgment.responses;
...
@SpringBootTest
class SaosJudgmentDetailsDeserializationTest {
    ...
    void shouldMapJudgmentDetails() {
        SaosJudgmentDetails judgmentDetails = new SaosJudgmentDetails();
        
        try {
            judgmentDetails = objectMapper.readValue(jsonDetails, SaosJudgmentDetails.class);
        } catch (JsonProcessingException e) {
            e.printStackTrace();
        }

        SaosJudgmentDetails finalJudgmentDetails = judgmentDetails;
        
        assertAll(
                () -> assertEquals( 20994L, finalJudgmentDetails.getSaosId()),
                () -> assertEquals( 3, finalJudgmentDetails.getSaosReferencedRegulations().size()),
                () -> assertEquals( 3, finalJudgmentDetails.getLegalBases().size()),
                () -> assertTrue( finalJudgmentDetails.getLegalBases().contains("art. 23 k.c.")),
                () -> assertEquals( 1, finalJudgmentDetails.getKeywords().size()),
                () -> assertTrue( finalJudgmentDetails.getKeywords().contains("dobra osobiste"))
        );
    }
